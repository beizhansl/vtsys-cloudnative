...
async def list_scanner_resource(
    ...
    """
    # 首先检查该node是否含有标签openvas_data=ready
    success = check_node_label(node_name=node_name, label_key="openvas_data", label_value="ready")
    if not success:
        return {
            "ok": False,
            "errmsg": f"Openvas data not ready on node {node_name}"
        }
    success = create_pod_from_yaml(node_name=node_name)
    if not success:
        return {
            "ok": False,
            "errmsg": f"Openvas scale failed on node {node_name}"
        }
    return {
        "ok": True,
        "errmsg": ""
